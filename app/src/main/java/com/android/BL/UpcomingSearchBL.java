...
public class UpcomingSearchBL {
    ...
    public String validate(String strValue) {
        ...
        if (strValue.equals("[]")) {
            System.out.println("newsed not found");
        } else {
            ...
            if (strValue.equals("[]")) {
                ...
                try {
                    ...
                    Constant.updescription = new String[jsonArrayObject.size()];
                    Constant.upnewsId = new String[jsonArrayObject.size()];
                    Constant.upimg = new String[jsonArrayObject.size()];
                    Constant.upnews_date = new String[jsonArrayObject.size()];

                    for (int i = 0; i < jsonArrayObject.size(); i++) {
                        JSONObject jsonObject = (JSONObject) jsonP.parse(jsonArrayObject.get(i).toString());
                        Constant.updescription[i] = jsonObject.get("description").toString();
                        Constant.upnewsId[i] = jsonObject.get("news_id").toString();
                        Constant.upimg[i] = jsonObject.get("image1").toString();
                        Constant.upnews_date[i] = jsonObject.get("event_date").toString();
                    }
